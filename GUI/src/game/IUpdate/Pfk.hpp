/*
** EPITECH PROJECT, 2023
** Zappy-Mirror
** File description:
** Pfk
*/

#pragma once
#include "IUpdate.hpp"

namespace GUI::Game {
    class Pfk : public GUI::Game::IUpdate {
        public:
            Pfk(std::shared_ptr<GUI::Game::GameState> gameState,
                std::shared_ptr<GUI::Network::IOPooledClient> client,
                std::string command)
            {
                _gameState = gameState;
                _client = client;
                _command = command;
            }

            ~Pfk(void) = default;

            void update(void) override
            {
                std::vector<std::string> args = GUI::Utils::StringUtils::split(_command, " ");

                if (args.size() != 2) {
                    std::cerr << "Error: Pfk: wrong number of arguments" << std::endl;
                    return;
                }
                if (args[0] != "pfk") {
                    std::cerr << "Error: Pfk: wrong command" << std::endl;
                    return;
                }
                std::shared_ptr<GUI::Game::Player> player = _findPlayerById(std::stoi(args[1]));
                if (!player) {
                    std::cerr << "Error: Pfk: player not found" << std::endl;
                    return;
                }
            }
        private:
            std::shared_ptr<GUI::Game::Player> _findPlayerById(int id)
            {
                for (auto &player : _gameState->getPlayers()) {
                    if (player->getId() == id) {
                        return player;
                    }
                }
                return nullptr;
            }
    };
}
